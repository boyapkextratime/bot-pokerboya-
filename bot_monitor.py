...
async def login_manual(update, context):
    await update.message.reply_text("Membuka browser... Silakan Login, setelah masuk, tutup browsernya.")
    async with async_playwright() as p:
        ...
        await update.message.reply_text("Login selesai, sesi disimpan!")
...
async def check_category(update, context):
    ...
    async with async_playwright() as p:
        ...
        for p_data in ALL_CATEGORIES[cat_name]['providers']:
            ...
            if len(results) % 5 == 0:
                await context.bot.edit_message_text(chat_id=update.effective_chat.id, message_id=msg.message_id, text=f"Sedang mengecek {cat_name}...\n\n" + "\n".join(results))
        
        final_text = f"Hasil Pengecekan {cat_name}:\n\n" + "\n".join(results)
